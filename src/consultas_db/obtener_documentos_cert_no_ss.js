...
const obtener_registros = async(param) => {
    ...
    if(param.tipo==='st8'){
        ...
        const documentos_obtenidos = await modelo.find({"Archivo":"enviado a certificar"}); // consulta y guarda todos los documentos encontrados con el parametro args
         
        var modelo = models['st8_st9'];

        const documentos_obtenidos2 = await modelo.find({"Archivo":"enviado a certificar"}); // consulta y guarda todos los documentos encontrados con el parametro args
        ...
        console.log('documentos obtenidos de consulta certificar.. todo:', documentos_obtenidos);
        console.log('longitud de todo',documentos_obtenidos.length)
